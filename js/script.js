...
function buyOnWhatsApp(productName) {
  ...
  // Mensaje predefinido
  const message = `¡Hola! Estoy interesad@ en comprar el producto: ${productName}.`;
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const kawaiiContainer = document.querySelector(
    "#kawaii-products .products-grid"
  );

  const products = await fetch("assets/products.json").then((res) =>
    res.json()
  );
  const renderProducts = (products, container, categoryClass) => {
    products.forEach((product) => {
      ...
      const imageSlides = images
        .map(
          (img, i) => `
      <img src="assets/img/products/${img}" alt="${
            product.name
    ...
    `
      ...
      <div class="product-info">
          ...
          <button class="whatsapp-btn" onclick="buyOnWhatsApp('${
            product.name
          }')">
  ...
  // Renderizar kawaii
  renderProducts(
    products.filter((p) => p.category === "kawaii"),
    kawaiiContainer,
    "kawaii-product"
  );
});
